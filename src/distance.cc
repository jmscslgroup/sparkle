    ...
    class distance: public SensorPlugin
    {
        ...
        public:
            ...
            double MADistance()
            {
                ...
                if(rv_length < N_POINTS-1)
                {
                    return (old_distance.at( old_distance.size() -1 ));
                }
                double sum = 0.0;
                for (int i = 0; i < old_distance.size() ; ++i)
                {
                    sum = sum + old_distance.at(i);
         ...
         //       ROS_INFO_STREAM("nanoSec: " << nanoSecs);
                ...
                if (oldtimedata != nowtime.toSec())
                {
                    ...
                    old_relvel.at(rv_length) = (min_dist - this->old_dist.data)/(2.2*elapsedTime);
                ...
                if (rv_length > 2)
                {
                    double acceleration = (old_relvel.at(rv_length) - old_relvel.at(rv_length -1) )/(elapsedTime);
                ...
                this->minimum_distance.data = old_distance.at( rv_length) - 4.5  ;
